      ...
      <div className="flex flex-col gap-4 px-2 pt-4 lg:grid lg:grid-cols-2 lg:px-0">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs sm:text-sm">Escanea el código QR del miembro</CardDescription>
          </CardHeader>
              ...
              <Card className="border-primary">
                <CardContent className="pt-4 sm:pt-6">
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:space-x-4 mb-4">
                    <Avatar src={scannedUser.photo} alt={scannedUser.name} className="w-16 h-16" />
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:space-x-4 mt-1">
                        <p className="text-xs sm:text-sm text-muted-foreground">
                          Último pago: {new Date(scannedUser.lastPayment).toLocaleDateString()}
                        </p>
                        <Badge status={scannedUser.status} className="text-xs sm:text-sm">
                          {scannedUser.status}
                        </Badge>
                      </div>
                    </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs sm:text-sm">Resumen de accesos del día</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-2 sm:p-4 bg-primary/5 rounded-lg">
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground">Total de Accesos</p>
                  <p className="text-xl sm:text-2xl font-bold text-primary">{logsLoading ? "..." : totalAccess}</p>
                </div>
              ...
              <div className="flex items-center justify-between p-2 sm:p-4 bg-destructive/5 rounded-lg">
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground">Accesos Denegados</p>
                  <p className="text-xl sm:text-2xl font-bold text-destructive">{logsLoading ? "..." : deniedAccess}</p>
                </div>
                <XCircle className="h-7 w-7 sm:h-8 sm:w-8 text-destructive" />
              </div>
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                  <p className="text-xs sm:text-sm text-muted-foreground">Hora Pico</p>
                  <p className="font-semibold text-sm sm:text-base">{logsLoading ? "..." : peakHourStr}</p>
                </div>
                <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                  <p className="text-xs sm:text-sm text-muted-foreground">Promedio/Hora</p>
                  <p className="font-semibold text-sm sm:text-base">{logsLoading ? "..." : avgPerHour.toFixed(1)}</p>
                </div>
              </div>
            </div>
          </CardContent>
      ...
      <Card className="mt-6 px-2 sm:px-0">
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
              <CardTitle className="text-base sm:text-lg">Registro de Accesos</CardTitle>
              <CardDescription className="text-xs sm:text-sm">Historial de entradas y salidas</CardDescription>
            </div>
                        ...
                        <TableRow key={log._id}>
                          <TableCell className="pl-2 sm:pl-4">
                            <div className="flex items-center gap-2 sm:space-x-3">
                              <Avatar src={log.avatar} alt={log.name} className="w-8 h-8" />
                              <span className="font-medium text-xs sm:text-sm">{log.name || "-"}</span>
                            </div>
                          </TableCell>
                          <TableCell className="flex">
                            <Badge status={log.status} className="text-xs sm:text-sm">{log.status}</Badge>
                          </TableCell>
